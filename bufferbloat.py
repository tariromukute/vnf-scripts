...
# behaviour.  For this experiment we want to investigate Reno Behaviour
...
class BBTopo(Topo):
    ...
    def build(self, n=2):
        ...
        self.addLink(h1, switch, bw=args.bw_net, delay=args.delay, max_queue_size=args.maxq)
        self.addLink(h2, switch, bw=args.bw_host, delay=args.delay, max_queue_size=args.maxq)
...
def start_iperf(net):
    ...
    client = h2.popen("iperf -c %s -t %s" % (h1.IP(), args.time + 10))
...
def start_ping(net):
    ...
    h1 = net.get('h1')
    h2 = net.get('h2')
    proc = h2.popen('ping -i 0.1 %s > %s/ping.txt' % (h1.IP(), args.dir), shell = True)
